...
from riverdata.models import GraniteFallsGauge, GraniteFallsPrediction, GraniteFallsForecast, \
    GraniteFallsPredictionArchive, GraniteFallsForecastLinear, GraniteFallsPredictionLinear, \
    GraniteFallsPredictionLinearArchive


class Command(BaseCommand):
    model = GraniteFallsForecastLinear
    ...
    def handle(self, *args, **options):
        try:
            ...
            granite_falls_forecast = list(
                GraniteFallsPredictionLinear.objects.filter(datetime__gt=current_datetime).values('datetime',
                                                                                            'gauge_name',
                                                                                            'stage',
                                                                                            'prediction_datetime'))
            ...
            self.stdout.write(self.style.SUCCESS('Successfully applied linear forecast to river data.'))

        except Exception as e:
            self.stdout.write(self.style.ERROR('Failed to apply linear forecast to river data: ' + str(e)))
    ...
    def archive_predictions(self):
        ...
        for entry in archive_prediction:
            GraniteFallsPredictionLinearArchive.objects.update_or_create(
